...
def main():
    ...
    if 'IMG' not in cfg.MODEL.BACKBONE.FUSION:
        norm_info_path = os.path.join(args.norm_info_folder,
                                      'norm_param_gt_fcos_coco_train_%02d.json' % cfg.DATASETS.RADAR_IMAGE_RADIUS)
        # norm_info_rcs_path = os.path.join(args.norm_info_folder,
        #                                   'norm_param_gt_fcos_coco_train_%02d_rcs.json' %
        #                                   cfg.DATASETS.RADAR_IMAGE_RADIUS)
        with open(norm_info_path, 'r') as f:
            norm_info = json.load(f)
        # with open(norm_info_rcs_path, 'r') as f:
        #     norm_info_rcs = json.load(f)
        cfg.INPUT.PC_MEAN = [norm_info['pc_im_means'][2], norm_info['pc_im_means'][1], norm_info['pc_im_means'][0]]
        # cfg.INPUT.PC_RCS_MEAN = [norm_info_rcs['pc_im_means'][2], norm_info_rcs['pc_im_means'][1], norm_info_rcs[
        # 'pc_im_means'][0]]
    cfg.freeze()
